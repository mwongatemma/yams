...
def code_cpu_n(host, cpu):
    data = get_cpu_n(host, cpu)

    js = """
    <button id="reset-cpu%d">Reset</button>
    <div id="cpu%d" style="width:600px;height:300px;">
    </div>""" % (cpu, cpu)

    js += """
    <script type="text/javascript" charset="utf-8">
      var options_cpu%d = {
          title: 'Processor %d Utilization',
          xaxis: {
              title: 'Time',
              tickFormatter: myDateFormatter
          },
      ...
      function myDraw() {
          function drawGraph(opts) {
              var o = Object.extend(Object.clone(options_cpu%d), opts || {});
              return Flotr.draw(
                  $('cpu%d'),
                  [
                      {
          ...
          $('cpu%d').observe('flotr:select', function(evt) {
              var area = evt.memo[0];
          ...
          $('reset-cpu%d').observe('click', function(){ drawGraph() });
      }

      document.observe('dom:loaded', myDraw);
    </script>""" % (cpu, cpu, cpu, cpu, data['idle'], data['interrupt'],
                    data['nice'], data['softirq'], data['steal'],
                    data['system'], data['user'], data['wait'], cpu, cpu)
...
WHERE time > :timestamp
  AND host = :name
  AND plugin_instance = :cpu
GROUP BY time
ORDER BY time ASC;"""), name=host, timestamp=timestamp, cpu=str(cpu))
    connection.execute('COMMIT;')
